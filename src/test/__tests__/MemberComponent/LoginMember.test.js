...
describe('Login Component', () => {
    ...
    test('logs in with valid credentials and remembers user', async () => {
        ...
        await waitFor(() =>
        expect(Cookies.set).toHaveBeenCalledWith('workEmail-member', 'user@example.com', { expires: 7 })
        );
        await waitFor(() =>
        expect(Cookies.set).toHaveBeenCalledWith('password-member', 'password@123', { expires: 7 })
        );
    });
    ...
    test('handles server error', async () => {
      ...
      test('submits form successfully if all inputs are valid', async () => {
        ...
        await waitFor(() => {
          expect(axios.post).toHaveBeenCalledWith('http://localhost:9000/codeVerification-member', expect.any(Object), { withCredentials: true });
        });
      });
